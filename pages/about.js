    ...
    const fetchAboutData = async () => {
      try {
        const response = await fetch(
          "https://202.44.12.87:1337/api/abouts?populate=uploadfiles.fileupload&populate=mission.uploadfiles"
        );
                ...
                <div className="sm:order-2 order-2">
                  <div className="relative h-[300px] sm:h-[400px] md:h-[500px] lg:h-[550px] xl:h-[550px] rounded-lg">
                    <img
                      ...
                      src={`https://202.44.12.87:1337${item.attributes.uploadfiles?.data[0]?.attributes?.url}`}
                      alt={
            ...
            item.attributes.topic === "Mission"
              ? item.attributes.mission.map((mission) => (
                  <CardMission
                    ...
                    // imageUrl={`https://202.44.12.87:1337${mission.uploadfiles?.data[0]?.attributes?.url}`} // Uncomment and use if an image URL is needed
